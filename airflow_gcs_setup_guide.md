...
1. Create a project folder, e.g. `airflow-gcs-demo`, and `cd` into it in VS Code's terminal.
...
3. Create the required folders and `.env` file (from the same doc):
   ...
   ```
4. **Give it 8GB RAM / 2 cores.** This is a Docker Desktop setting, not something you put in the compose file:
   - Open Docker Desktop → **Settings → Resources**
   - Set **Memory** to 8 GB and **CPUs** to 2
   - Apply & Restart Docker Desktop
5. Add the Google provider package so the GCS hook is available. Easiest way for local dev — add this line to `docker-compose.yaml` under the `x-airflow-common.environment` section:
   ```yaml
...
6. Initialize and start:
   ...
   docker compose up -d
   ```
7. Wait a minute or two, then check all services are healthy:
   ```bash
   docker compose ps
   ```
8. Open the UI at [http://localhost:8080](http://localhost:8080) and log in with `airflow` / `airflow`.
...
Create `./dags/hello_world_gcs.py`:
...
4. Optionally confirm in the GCS console (or `gsutil ls gs://your-bucket-name/`) that `hello_world.txt` exists.

---

## Troubleshooting tips

- If the DAG doesn't appear in the UI, check `docker compose logs airflow-scheduler` for import errors (often a missing provider package — re-check Step 1.5 and re-run `docker compose up -d` after editing `_PIP_ADDITIONAL_REQUIREMENTS`, since it reinstalls on container start).
